...
const AREA_LABELS = {
  ...
  sem_dados: { label: 'Sem dados', class: 'bg-muted text-gray-500' },
}
        ...
        <table className="min-w-full leading-normal">
          <thead>
            <tr>
              ...
              <th className="px-5 py-3 border-b-2 border-border bg-muted text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden md:table-cell">
                Saúde
              </th>
              <th className="px-5 py-3 border-b-2 border-border bg-muted text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden md:table-cell">
                Educação
              ...
              <tr key={child.id} className="hover:bg-accent transition-colors">
                <td className="px-5 py-4 border-b border-border bg-card text-sm">
                  <Link href={`/children/${child.id}`} className="flex items-center gap-2 group">
                    ...
                    <div>
                      <p className="text-foreground font-medium group-hover:text-rio-blue transition-colors">{child.nome}</p>
                      <p className="text-gray-500 text-xs">{calcularIdade(child.dataNascimento)} anos</p>
                    </div>
                  </Link>
                </td>
                <td className="px-5 py-4 border-b border-border bg-card text-sm">
                  <p className="text-muted-foreground">{child.bairro}</p>
                </td>
                <td className="px-5 py-4 border-b border-border bg-card text-sm hidden md:table-cell">
